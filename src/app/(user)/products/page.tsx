...
export default function ProductsPage() {
  ...
  // ── Filter & Sort ──
  // Helper: ambil category id dari produk (handle array atau single object)
  const getCatId = (p: Product): string | null => {
    const cat = p.product_categories;
    if (!cat) return null;
    if (Array.isArray(cat)) return cat[0]?.id ?? null;
    return (cat as any).id ?? null;
  };
  ...
  const filteredProducts = products
    .filter((p) => {
      ...
      return getCatId(p) === selectedCategory;
    })
    .sort((a, b) => {
            ...
            {filteredProducts.map((p) => {
              ...
              return (
                <Card
                  ...
                  className="group relative bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer !py-0 !gap-0"
                >
                  {/* Gambar */}
                  ...
                  <div className="p-2.5 sm:p-3 flex flex-col flex-1">
                    ...
                    <Tooltip delayDuration={300}>
                      <TooltipTrigger asChild>
                        <Link href={`/products/${p.slug}`} className="block overflow-hidden">
                          <h2 className="text-[12px] sm:text-sm font-semibold line-clamp-2 leading-snug hover:text-emerald-600 transition">
                            {p.name}
                          </h2>
                        </Link>
                      </TooltipTrigger>
                    ...
                    <div className="mt-auto pt-2 sm:pt-3 border-t border-slate-50">
                      ...
                      {/* Tombol Beli Sekarang — selalu tampil mobile & desktop */}
                      <Button
                        disabled={isOutOfStock || cartLoadingId === p.id}
                        onClick={(e) => handleCartClick(e, p)}
                        className="w-full h-9 text-[11px] sm:text-xs font-semibold rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white transition-colors shadow-sm"
                      >
...
function ProductCardSkeleton() {
  return (
    <Card className="bg-white border border-slate-200 rounded-xl overflow-hidden !py-0 !gap-0">

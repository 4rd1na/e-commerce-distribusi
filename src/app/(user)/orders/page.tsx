...
interface Order {
  id: string;
  status: "pending" | "waiting_payment" | "paid" | "processed" | "shipped" | "completed" | "cancelled";
  total: number;
  created_at: string;
  items: OrderItem[];
}
...
export default function OrdersPage() {
  ...
  const filteredOrders = orders.filter((order) => {
    ...
    if (activeTab === "progress") {
      return ["pending", "waiting_payment", "paid", "processed", "shipped"].includes(order.status);
    }
    return true;
  });
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 ? (
          ...
          <div className="text-center py-12 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
            ...
            <p className="text-sm font-medium text-slate-500">Tidak ada transaksi ditemukan</p>
          </div>
        ) : (
            ...
            return (
              ...
              >
                ...
                <div className="px-4 py-3.5 bg-slate-50 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  ...
                  <div className={`self-start sm:self-auto flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium ${config.className}`}>
                    {config.icon}
                    {config.label}
                  </div>
                </div>
                    ...
                    <div key={index} className="p-4 flex gap-4 items-start">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5 mt-1 text-xs text-slate-500">
                          {item.variant && (
                            <>
                              <span>Varian: <strong className="text-slate-700 font-medium">{item.variant}</strong></span>
                              <span className="text-slate-300">|</span>
                            </>
                          )}
                          <span>Jumlah: <strong className="text-slate-700 font-medium">{item.qty}</strong></span>
                        </div>
                      ...
                      <div className="text-right shrink-0 hidden sm:block">
                        <div className="text-xs text-slate-400 mb-0.5">Harga Satuan</div>
                ...
                <div className="px-4 py-3.5 bg-white border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-6 border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100">
                    <div>
                      <div className="text-[11px] text-slate-400 text-left sm:text-right">Total Belanja</div>
